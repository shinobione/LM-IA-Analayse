...
echo Les couches GPU restent optionnelles: elles ne peuvent pas casser V2-A.
echo.
...
echo [5/6] Verification V2-B Neural + V2-D Stems / CUDA...
set "NEURAL_READY=0"
set "STEMS_READY=0"
...
"%VENV%\Scripts\python.exe" -c "import torch,transformers; from transformers import ClapModel,ClapProcessor; assert torch.cuda.is_available(); x=torch.randn((128,128),device='cuda'); y=x@x; torch.cuda.synchronize(); print('[OK] V2-B READY | Torch',torch.__version__,'| CUDA',torch.version.cuda,'| Transformers',transformers.__version__,'| GPU',torch.cuda.get_device_name(0))"
if errorlevel 1 goto :neural_warning
set "NEURAL_READY=1"

:stems_setup
echo [V2-D 1/2] Verification Demucs + torchaudio CUDA...
"%VENV%\Scripts\python.exe" -c "import torch,torchaudio,demucs; assert torch.cuda.is_available()" >nul 2>&1
if errorlevel 1 (
  echo [V2-D 1/2] Installation torchaudio CUDA 12.8...
  ...
  echo [V2-D 1/2] Installation Demucs...
  "%UV%" pip install --python "%VENV%\Scripts\python.exe" --upgrade -r "%BACKEND%\requirements-stems.txt"
  if errorlevel 1 goto :stems_warning
)

echo [V2-D 2/2] Test moteur stems GPU...
"%VENV%\Scripts\python.exe" -c "import torch,torchaudio,demucs; assert torch.cuda.is_available(); print('[OK] V2-D READY | Demucs htdemucs | GPU',torch.cuda.get_device_name(0))"
if errorlevel 1 goto :stems_warning
set "STEMS_READY=1"
goto :launch
...
:neural_warning
echo.
echo [WARN] La couche Neural V2-B n'est pas disponible. V2-A reste fonctionnelle.
set "NEURAL_READY=0"
goto :stems_setup

:stems_warning
echo.
echo [WARN] La couche Stems V2-D n'est pas disponible sur ce noeud.
echo [WARN] Le cluster peut encore utiliser un worker LAN compatible.
set "STEMS_READY=0"
...
if "%STEMS_READY%"=="1" echo [OK] V2-D Demucs GPU PRET.
