...
class GPUSampler:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=worker, name="sonictrace-larger-clap-gpu-sampler", daemon=True)
        self._thread.start()
...
def _encode_text_axes(model: Any, processor: Any, taxonomy: dict[str, list[dict[str, str]]], device: str) -> dict[str, Any]:
    import torch
...
def _encode_audio_track(
    ...
    *,
    ...
    for start in starts:
        ...
        # Exact native 10-second arrays neutralize the official processor's
        # rand_trunc path. Truth/benchmark metadata never participates here.
        segment = np.asarray(segment, dtype=np.float32)
        if segment.size != clip_samples:
            raise RuntimeError(f"Candidate D staging produced non-exact clip length: {segment.size}")
        inputs = processor(
            audios=segment,
        ...
        embeddings.append(F.normalize(emb.float(), dim=-1).squeeze(0).detach().cpu())
        if device.startswith("cuda"):
            torch.cuda.empty_cache()

    if not embeddings:
        raise RuntimeError(f"No LAION Larger CLAP Music audio embedding produced for: {audio}")
    ...
    meta = {
        ...
        "official_processor_rand_trunc_neutralized": True,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run SonicTrace V4 LAION Larger CLAP Music audio-only benchmark.")
    ...
    if not device.startswith("cuda"):
        raise SystemExit("LAION Larger CLAP Music Model Lab requires an NVIDIA CUDA GPU for this benchmark.")
    ...
    print(f"Model: {MODEL_ID} @ {MODEL_REVISION} | 48 kHz | {args.clips} deterministic exact 10s clips/track")
    print("License boundary: Apache-2.0")
    print("Inference input: AUDIO ONLY. TXT metadata is benchmark-only.\n")
    print("[INIT] Loading LAION Larger CLAP Music. First run may download ~776 MB...")

    processor = ClapProcessor.from_pretrained(MODEL_ID, revision=MODEL_REVISION)
    model = ClapModel.from_pretrained(MODEL_ID, revision=MODEL_REVISION).to(device).eval()
    print("[OK] Larger CLAP Music loaded. Precomputing generic taxonomy text embeddings...")
    text_embeddings = _encode_text_axes(model, processor, taxonomy, device)
    ...
    for audio in args.audio:
        ...
        try:
            ...
            # only after every audio-only axis ranking is complete.
        ...
        print(f"BENCHMARK : {evaluation['status']} | {elapsed:.1f}s | peak VRAM {sampler.peak_mib} MiB")
    ...
    payload = {
        "schema": "sonictrace-v4-model-lab-larger-clap-music-result-v1",
        ...
        "model": {
            "name": "LAION Larger CLAP Music",
            "id": MODEL_ID,
            "revision": MODEL_REVISION,
            "embedding_dimension": dimension,
            "sample_rate": SAMPLE_RATE,
            "clip_seconds": CLIP_SECONDS,
            "audio_policy": "5 deterministic evenly spaced exact native 10-second clips; normalized embedding mean",
            "code_license": "Apache-2.0",
            "weights_license": "Apache-2.0",
            "license_gate": "Apache-2.0 checkpoint/model card; preserve attribution/license compliance before product shipping.",
        },
        "gpu": gpu,
        "tracks": tracks,
    }
    ...
    json_path = args.results / f"larger-clap-music-benchmark-{stamp}.json"
    txt_path = args.results / f"larger-clap-music-benchmark-{stamp}.txt"
