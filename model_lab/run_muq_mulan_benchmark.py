...
class GPUSampler:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=worker, name="sonictrace-muq-gpu-sampler", daemon=True)
        self._thread.start()
...
def _clip_starts(total_samples: int, *, clips: int) -> list[int]:
    ...
    if max_start == 0 or clips <= 1:
        return [0]
    # Deterministic coverage of the entire song. No energy-based or metadata-based
    # cherry-picking: intro, interior positions and late-song material all vote.
...
def _coerce_embeddings(value: Any, *, expected_batch: int, torch_module: Any) -> Any:
    if not torch_module.is_tensor(value):
        raise TypeError(f"MuQ-MuLan returned unsupported embedding type: {type(value).__name__}")
    tensor = value.float()
    if tensor.ndim == 1:
        tensor = tensor.unsqueeze(0)
    if tensor.ndim != 2 or tensor.shape[0] != expected_batch:
        raise RuntimeError(f"Unexpected MuQ-MuLan embedding shape: {tuple(tensor.shape)}")
    return tensor
...
def _encode_audio_track(model: Any, audio: Path, *, clips: int, device: str) -> tuple[Any, dict[str, Any]]:
    import librosa
    import numpy as np
    import torch
    import torch.nn.functional as F
    ...
    for start in starts:
        ...
        wavs = torch.from_numpy(np.asarray(segment, dtype=np.float32)).unsqueeze(0).to(device=device, dtype=torch.float32)
        with torch.no_grad():
            raw = model(wavs=wavs)
...
def _write_text_report(path: Path, payload: dict[str, Any]) -> None:
    lines = [
        "SONICTRACE V4 MODEL LAB — MuQ-MuLan 700M",
        "=" * 58,
    ...
    for track in payload["tracks"]:
        ...
        lines.extend([
            ...
            f"Clips: {segment['clip_count']} x {segment['clip_seconds']:.0f}s @ 24kHz | starts: {segment['clip_start_seconds']}",
        ])
        for axis in AXES:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run SonicTrace V4 Model Lab MuQ-MuLan audio-only benchmark.")
    ...
    cache_dir = runtime_dir / "huggingface"
    cache_dir.mkdir(parents=True, exist_ok=True)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    if device != "cuda":
        raise SystemExit("MuQ-MuLan Model Lab requires an NVIDIA CUDA GPU for this benchmark.")
    ...
    print(f"Model: {MODEL_ID} | fp32 | 24 kHz | {args.clips} deterministic 10s clips/track")
    print("Inference input: AUDIO ONLY. TXT metadata is benchmark-only.\n")
    print("[INIT] Loading MuQ-MuLan. First run may download several GB...")

    torch.set_grad_enabled(False)
    model = MuQMuLan.from_pretrained(MODEL_ID, cache_dir=str(cache_dir))
    model = model.to(device).eval()
    model = model.float()
    print("[OK] MuQ-MuLan loaded. Precomputing generic taxonomy text embeddings...")
    text_embeddings = _encode_text_axes(model, taxonomy, device)
    print("[OK] Taxonomy embeddings ready.\n")

    report: dict[str, Any] = {
        "schema": "sonictrace-v4-model-lab-muq-result-v1",
        ...
        "model": {
            ...
            "sample_rate": SAMPLE_RATE,
            "clip_seconds": CLIP_SECONDS,
            "audio_policy": f"{args.clips} deterministic evenly spaced 10-second clips; normalized embedding mean",
    ...
    for requested in args.audio:
        ...
        try:
            ...
            torch.cuda.empty_cache()
            raise RuntimeError(
                "MuQ-MuLan ran out of RTX VRAM. The model stays fp32 as recommended; reduce --clips only if the implementation is later changed to batch clips."
            ) from exc
        finally:
            sampler.stop()

        elapsed_total = time.perf_counter() - started
        # Benchmark truth is loaded only after audio inference and all four axis
        # rankings are complete. It cannot influence embeddings or ranking.
    ...
    json_path = results_dir / f"muq-mulan-benchmark-{stamp}.json"
    txt_path = results_dir / f"muq-mulan-benchmark-{stamp}.txt"
