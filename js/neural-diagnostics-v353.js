(() => {
  ...
  const VERSION = '3.5.3.1-diagnostic';
  ...
  window.fetch = async function sonicTraceDiagnosticFetch(...args) {
    ...
    if (isNeuralAnalyze(url) && response?.ok && typeof response.clone === 'function') {
      response.clone().json().then(payload => {
        latestPayload = payload;
        document.documentElement.dataset.sonictraceNeuralDiagnosticCapture = 'captured';
        scheduleRender();
      }).catch(error => {
        document.documentElement.dataset.sonictraceNeuralDiagnosticCapture = 'error';
  ...
  function scheduleRender() {
    ...
    [0, 120, 450, 1000].forEach(delay => window.setTimeout(render, delay));
  ...
  function styleList(rows, mode) {
    ...
    return source.map(row => {
      ...
      const score = mode === 'ensemble'
        ? (Number.isFinite(Number(row?.ensemble_percent))
          ? `${Math.round(Number(row.ensemble_percent) * 10) / 10}%`
          : percent(scoreOf(row, ['ensemble_score'])))
  ...
  function ensurePanel(context) {
    ...
    // Put the probe where the user is already looking: immediately after the
    // Neural style badges and before the Lyrics block. Never bury it after TXT
    // metadata at the bottom of the context card.
    const tags = context.querySelector('.semantic-context-tags');
    if (tags && panel.previousElementSibling !== tags) tags.insertAdjacentElement('afterend', panel);
    else if (!panel.isConnected) context.prepend(panel);
    return panel;
  }
  ...
  function render() {
    ...
    panel.innerHTML = `
      <style>
        .sonictrace-neural-diagnostic{margin:12px 0 16px;padding:13px;border:1px solid rgba(38,229,206,.38);border-radius:12px;background:rgba(3,22,24,.9);font-size:12px;line-height:1.45;box-shadow:0 0 0 1px rgba(38,229,206,.05) inset}
        .sonictrace-neural-diagnostic h4{margin:0 0 3px;color:#5ff5df;font-size:13px;text-transform:uppercase;letter-spacing:.07em}
        .sonictrace-neural-diagnostic>p{margin:0 0 10px;color:#8ca0a8}.diag-captured{color:#4df0c8;font-weight:800}
        .diag-grid{display:grid;grid-template-columns:1fr 1fr;gap:9px}.diag-card{padding:9px;border:1px solid rgba(130,164,173,.16);border-radius:9px;background:rgba(5,13,16,.62);min-width:0}
        .diag-card strong{display:block;color:#dbe9ec;margin-bottom:5px;font-size:11px;text-transform:uppercase;letter-spacing:.05em}.diag-card>span,.diag-list span{display:block;color:#b6c8cd;margin:3px 0;overflow-wrap:anywhere}
        .diag-list b{color:#eef8fa}.diag-list small{color:#6f858d}.diag-empty{color:#6f858d!important}.diag-wide{grid-column:1/-1}.diag-kv b{color:#5ff5df}
        @media(max-width:760px){.diag-grid{grid-template-columns:1fr}.diag-wide{grid-column:auto}}
      </style>
      <h4>Diagnostic Neural ${VERSION} · <span class="diag-captured">PAYLOAD CAPTURÉ</span></h4>
      <p>Lecture seule · vrai payload du scan · le TXT ne modifie pas l’inférence.</p>
      <div class="diag-grid">
        ...
        <div class="diag-card diag-wide diag-kv"><strong>3 · Family cluster V3.5.2</strong>
          ...
          <span>Runner-up ${esc(runner)}</span><span>Labels soutien : ${esc(labels)}</span>
          <span>Rôles : ${esc(roles)} · labels comptés ${esc(cluster.label_count ?? '—')} · eligible ${esc(cluster.eligible ?? '—')}</span>
        </div>
        <div class="diag-card diag-kv"><strong>4 · Décision</strong>
          <span>Primary analysis <b>${esc(rawPrimary || '—')}</b></span><span>Primary ensemble <b>${esc(ensemblePrimary || '—')}</b></span>
          <span>Décision ensemble <b>${esc(ensemble.decision || '—')}</b></span><span>Style dimensions <b>${esc(resolvedStyle)}</b></span><span>Famille dimensions <b>${esc(resolvedFamily)}</b></span>
        </div>
        <div class="diag-card diag-kv"><strong>5 · Expert / versions</strong>
          <span>Tradition <b>${esc(tradition.primary?.label || '—')}</b> · forme <b>${esc(form.primary?.label || '—')}</b></span>
          <span>Discogs famille #1 <b>${esc(expertFamily)}</b></span><span>Neural <b>${esc(neural.engine?.analysis_version || '—')}</b> · ensemble <b>${esc(ensemble.version || '—')}</b></span>
  ...
  const observer = new MutationObserver(() => {
    if (!latestPayload) return;
    const context = [...document.querySelectorAll('.semantic-context')].at(-1);
    const panel = context?.querySelector('[data-sonictrace-neural-diagnostic]');
    const tags = context?.querySelector('.semantic-context-tags');
    if (!panel || (tags && panel.previousElementSibling !== tags)) scheduleRender();
  });
  observer.observe(document.documentElement, { childList: true, subtree: true });

  window.LMNNeuralDiagnostics = Object.freeze({ version: VERSION, render, getPayload: () => latestPayload });
